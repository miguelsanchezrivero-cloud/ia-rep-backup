...
import { estimateAudienceSize, filterDoctors, filterPharmacy } from '../lib/audience'
...
interface AppState {
  ...
  startDoctorVisit: (campaignId: string, doctorId: string, channel?: Channel) => void
...
function msg(role: ConversationMessage['role'], content: string, extra?: Partial<ConversationMessage>): ConversationMessage {
  return {
    id: uid('m'),
    role,
    content,
...
export const useAppStore = create<AppState>((set, get) => ({
  ...
  upsertAvatar: (avatar) =>
    set((s) => {
      ...
      return {
        avatars: exists ? s.avatars.map((a) => (a.id === avatar.id ? avatar : a)) : [...s.avatars, avatar],
      }
    }),
      ...
      campaigns: s.campaigns.map((c) =>
        c.id === id
          ? {
              ...c,
              status,
              testedAt: status === 'testing' || status === 'approved' ? c.testedAt ?? new Date().toISOString() : c.testedAt,
  ...
  dispatchCampaign: (campaignId, channel) => {
    ...
    if (campaign.status !== 'approved' && campaign.status !== 'live') {
      return { ok: false, message: 'La campaña debe estar aprobada (go final) antes del envío' }
    }
    const recipients = estimateAudienceSize(campaign, s.doctors, s.pharmacyStaff)
    if (!recipients) return { ok: false, message: 'La audiencia filtrada está vacía' }
    const cost = recipients * s.credits.costPerVisit
    if (s.credits.balance < cost) {
      return { ok: false, message: `Saldo insuficiente. Necesita ${cost} créditos` }
    }
    ...
    set({
      ...
      campaigns: s.campaigns.map((c) => (c.id === campaignId ? { ...c, status: 'live' as const } : c)),
      analytics: {
        ...s.analytics,
  ...
  startTestSession: (campaignId) => {
    ...
    const campaign = s.campaigns.find((c) => c.id === campaignId)
    if (!campaign) return
    const avatar = s.avatars.find((a) => a.id === campaign.avatarId) ?? s.avatars[0]
    ...
    session.messages = [
      msg('system', 'Modo prueba — valide script, objeciones y límites de gobernanza'),
    ...
    set({
      testSession: session,
      campaigns: s.campaigns.map((c) =>
        c.id === campaignId ? { ...c, status: c.status === 'draft' ? 'testing' : c.status, testedAt: new Date().toISOString() } : c,
      ),
    })
  },
  ...
  sendTestMessage: (text) => {
    ...
    const realRep = doctor ? s.realReps.find((r) => r.id === doctor.realRepId) ?? null : null
  ...
  startDoctorVisit: (campaignId, doctorId, channel = 'whatsapp') => {
    ...
    const session: VisitSession = {
      ...
      ctaClicks: [],
      materialsRequested: [],
      contextNotes: doctor.birthday === '08-12' ? ['Cumpleaños hoy'] : ['Tráfico moderado en sector'],
    }
  ...
  startPharmacyVisit: (campaignId, pharmacyId) => {
    ...
    session.messages = [msg('avatar', opening.content, { sources: opening.sources })]
  ...
  sendVisitMessage: (text) => {
    ...
    const avatar = s.avatars.find((a) => a.id === session.avatarId)!
    const doctor = s.doctors.find((d) => d.id === session.targetId) ?? null
    const pharmacy = s.pharmacyStaff.find((p) => p.id === session.targetId) ?? null
    const realRep = doctor ? s.realReps.find((r) => r.id === doctor.realRepId) ?? null : null
  ...
  clickCta: (type, productId) => {
    ...
    const updated: VisitSession = {
      ...session,
      ctaClicks: [...session.ctaClicks, { type, at: new Date().toISOString(), productId }],
      materialsRequested:
    ...
    set({
      ...
      analytics: {
        ...
        samplesRequested: s.analytics.samplesRequested + (type === 'sample_request' ? 1 : 0),
      },
    } as Partial<AppState>)
  },
...
export function getCampaignAudienceCount(campaign: Campaign) {
  ...
  if (campaign.audience === 'pharmacy_staff') return filterPharmacy(s.pharmacyStaff, campaign).length
