...
export function Card({
  ...
  interactive = false,
  onClick,
}: PropsWithChildren<{ className?: string; interactive?: boolean; onClick?: () => void }>) {
  return (
    <div
      className={cn(
        'rounded-2xl border border-ink-200/80 bg-white shadow-sm shadow-ink-900/5',
        interactive && 'cursor-pointer transition-all duration-200 hover:-translate-y-1 hover:shadow-lg',
        className,
      )}
      onClick={onClick}
    >
...
export function Modal({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center">
      ...
      <div
        className={cn(
          'relative w-full max-w-md transform rounded-2xl bg-white p-6 shadow-2xl transition-all duration-350',
      ...
      >
        ...
        {actions ? <div className="mt-6 flex flex-wrap gap-2 justify-end">{actions}</div> : null}
      </div>
    </div>
  )
}
    ...
    <div className="fixed inset-0 z-50">
      ...
      >
        <div className="flex h-full flex-col">
          ...
          {action ? <div className="border-t border-ink-100 px-6 py-4">{action}</div> : null}
        </div>
      </div>
    </div>
  )
